/**
 * Script para corregir imágenes de galería "personaje"
 *
 * Patrón: personaje (srcset inverso)
 * - PC: 105px
 * - Móvil: 200px
 * - Sufijo: -desktop.webp
 * - srcset: "{img}-desktop.webp 105w, {img}.webp 200w"
 * - sizes: "(max-width: 900px) 200px, 105px"
 *
 * Páginas afectadas: /colores/, /my-hero-academia/
 * (/dragon-ball/ ya está corregida)
 */
...
// Páginas con "personaje" pendientes de corregir
...
// Extraer imágenes de "personaje" (no personaje6/7/8) SIN srcset
function extractPersonajeImagesWithoutSrcset(html) {
  const images = [];

  // Buscar imágenes dentro de <a class="personaje"> (exacto, no personaje6/7/8)
  // El patrón busca class="personaje" seguido de espacio o comilla de cierre
  const regex = /<a class="personaje"[^>]*>\s*<img([^>]*)src="([^"]+)"([^>]*)>/g;
...
async function processImage(relativePath) {
  ...
  try {
    ...
    // 2. Crear versión desktop (105px) si no existe
    if (!await fileExists(desktopPath)) {
      await sharp(originalBuffer)
        .resize(105, null, { withoutEnlargement: true })
        .webp({ quality: QUALITY })
        .toFile(desktopPath);
...
function updateHtmlWithSrcset(html, images) {
  ...
  for (const img of images) {
    ...
    const srcset = `/uploads/${dir}/${base}-desktop.webp 105w, /uploads/${dir}/${base}.webp 200w`;
    const sizes = '(max-width: 900px) 200px, 105px';
...
async function processPage(pageRelativePath) {
  ...
  const images = extractPersonajeImagesWithoutSrcset(html);
...
async function main() {
  ...
  console.log('CORRECCIÓN DE IMÁGENES "PERSONAJE"');
  console.log('='.repeat(70));
  console.log('\nPatrón: srcset inverso (105px desktop, 200px móvil)');
  ...
  console.log(`Espacio ahorrado: ${formatBytes(stats.bytesSaved)}`);
